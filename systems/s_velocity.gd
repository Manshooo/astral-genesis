...
func query() -> QueryBuilder:
	return q.with_all([C_Velocity]).iterate([C_Velocity]) # Use q.with_all([YourComponent])
...
func process(entities: Array[Entity], components: Array, delta: float) -> void:
	...
	for entity in entities:
		var position: Vector3 = entities[entity].transform.origin
		position += velocities[entity].velocity * delta
		entities[entity].transform.origin = position
