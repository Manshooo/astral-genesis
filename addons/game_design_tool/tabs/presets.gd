...
func _build_toolbar() -> Control:
	...
	bar.add_child(Ui.button("Проверить сцены", _on_validate_pressed))
...
func _build_lists() -> Control:
	...
	tags_box.add_child(Ui.section_label("Словарь тегов"))
	_tag_tree = Tree.new()
	_tag_tree.columns = 2
