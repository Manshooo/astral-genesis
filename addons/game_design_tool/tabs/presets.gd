...
## правки генерации:
##   1. веса/slot_count пресетов разбросаны по .tres рядом со своими сценами
##      (src/levels/procedural/rooms/) — здесь они в одной таблице и правятся
##      на месте; теги — отдельным облаком под таблицей (см. ниже, почему не
##      прямо в ячейке);
...
##      Это НЕ Room Wizard (см. док `addons/game_design_tool/dock/
##      room_wizard.gd`) — тот заводится от УЖЕ нарисованной сцены и
##      назначает scene сразу; «Новый пресет» — для обратного порядка,
##      планируешь пресет раньше, чем нарисована комната;
