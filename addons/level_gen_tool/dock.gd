## res://addons/level_gen_tool/dock.gd
## Док-редактор генератора уровня. Закрывает три боли правки генерации:
...
## RunManager раскладывает слой, иначе инструмент проверял бы не то, что делает игра.
@tool
...
func _init() -> void:
	name = "Генератор"
	# Только ширина и по минимуму: полную минимальную высоту дока редактор
	# требует у ВСЕХ вкладок сразу, ещё до того как их открыли, — раздутая
	# минималка разъезжает всю правую панель (см. про статус-строку в _build_ui).
...
func _build_ui() -> void:
	...
	# Статус — строго ОДНА строка с многоточием, а не autowrap. Label с
	# autowrap считает минимальную высоту, перенося текст по минимальной ШИРИНЕ
	# (а она до первой раскладки ~17 px), и требует под себя больше тысячи
	# пикселей. Док отдаёт эту минималку редактору ещё до того, как вкладку
	# открыли, — правая панель разъезжается и нижнюю не видно. Полный текст
