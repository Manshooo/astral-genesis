...
- **Vertical edges go to portals, not doors.** `_bind_portals` runs first and hands every edge that changes `depth` to an `E_VerticalPortal` in the room (that is what `vertical_hub_*` presets carry one for); doors only get what is left. A room has one portal but can have several vertical edges, so the surplus still falls back to doors. `A_UsePortal extends A_TravelThroughDoor` — a portal is mechanically the same edge traversal, it only overrides the wording, so the lock/sealed checks cannot drift between them.
...
An interactable object needs three things: a **`C_Interactable`** component, a collider on the **`interactives` physics layer (layer_4, mask bit 8)**, and either an `interact()` method or `actions[]` of `RS_InteractionAction`. `S_InteractionDetector` raycasts from the player camera each physics frame and walks *up* the node tree from the hit collider to find the owning `Entity` (so the collider may live deep inside an imported scene, not on the Entity itself). It toggles `C_Highlighted`, which drives crosshair growth and the outline observer (`O_OutlineVisual`). A **body-snatch target** is detected separately by `S_SnatchTargetDetector` (same pattern, but the `enemies` layer and `capture_range`) and marked with **`C_SnatchTargeted`** — a distinct component precisely because `C_Highlighted` would drag the outline and the prompt along with it. `S_BodySnatch` reads that mark instead of casting its own ray (it declares `Runs.Before` via `deps()`), so "what we aim at" and "what the player sees highlighted" cannot drift apart. Outline only finds meshes that are a `GeometryInstance3D` root or a child named exactly `MeshInstance3D` — which is why `O_BodyVisual` swaps only `mesh`/`material_override` on that node instead of replacing it.

`C_Interactable.requires_body` marks physical mechanisms (levers, buttons) the disembodied БФЖ cannot operate: `S_InteractInput` swallows the press and the prompt explains why. It defaults to **false** — doors, portals and the incubator must stay usable, since a run *starts* disembodied and would otherwise dead-lock.

Two distinct text channels — don't conflate them:
- **Prompt** (`hud_prompt.gd`) — persistent while the crosshair is on the object. Shows `C_Interactable.prompt_text`, prefixed with the key resolved from `C_Interactable.action_name` via `SettingsManager.action_display_name()` — so it is right for any action (`snatch_body` reads «ЛКМ», not «F») and re-renders on `settings_changed`, i.e. immediately after a rebind. Suppressed by `show_key_hint = false` (objects that explain themselves but do nothing when pressed) and replaced by «Нужно тело» when `requires_body` and the player is a ghost.
...
- Physics layers: 1 `static_colliders`, 2 `player`, 3 `enemies`, 4 `interactives`, 5 `moving_colliders`, 6 `permeable`. Systems reference these by bit (e.g. body-snatch raycasts only `enemies` = `1 << 2`).
- **`permeable` (layer 6)** is geometry the disembodied БФЖ phases through — grilles, thin partitions. Content opts in by putting the collider on that layer instead of `static_colliders`; `S_PlayerMovement._set_phasing` drops the bit from the *player's mask* while there is no `C_Embodied`. Load-bearing walls and floors stay on `static_colliders` and solid for everyone: rooms sit 60 m apart in a void, so a ghost leaving the room would fall out of the level and bypass the whole graph.
- **Movement has two modes** in `S_PlayerMovement`, split by `C_Embodied`: embodied is the ordinary shooter walk (gravity, jump, instant start/stop); disembodied is flight — no gravity at all, **no jump** (there is nothing to jump with, and looking up/down already moves you vertically), direction taken from the **camera** basis, and velocity eased toward the target via `move_toward` so the ghost drifts. Tunables live in `RS_GameConfig` under «БФЖ (развоплощён)». The ghost branch still clears `jump_pressed`, or the press would fire the instant the soul enters a body. That "which actions exist depends on what you are" is an `if/else` today; per-body ability sets (a legless body that cannot jump) are designed in «Возможности тела и раскладка управления» (v0.5.0).
