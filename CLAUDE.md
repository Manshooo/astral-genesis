...
- **Movement has two modes** in `S_PlayerMovement`, split by `C_Embodied`: embodied is the ordinary shooter walk (gravity, jump, instant start/stop); disembodied is flight — no gravity at all, direction taken from the **camera** basis (look down, fly down, so no extra keys are needed), and velocity eased toward the target via `move_toward` so the ghost drifts. Tunables live in `RS_GameConfig` under «БФЖ (развоплощён)».
