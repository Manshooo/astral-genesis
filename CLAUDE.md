...
- **Testing and debugging** is the **`gameplay-testing`** skill's job — it writes and runs the headless checks in `dev/` (`godot --headless <scene>`, they exit nonzero on failure) and carries the manual-playtest checklist. Invoke it rather than reinventing a run command; there is no third-party test framework (GUT/gdUnit) in this project, the checks are plain Godot scenes on a shared harness (`dev/check_harness.gd`), all of them run by `dev/run_checks.ps1` locally and by the «Headless-проверки» job in CI. For what headless cannot judge — timings, sound, feel — the state for a live run is set up by the **debug overlay** (`dev/debug_overlay.tscn`, F1): the skill screen anywhere, layer teleport, decay off, and in its debug menu (`` ` ``) skill points, Architect essence and teleport to unique rooms. It lives in `dev/` and is loaded by path, never `preload`, because `dev/*` is excluded from export — no file must mean «no cheats», not a broken build. [Отладочный оверлей](docs/astral-genesis/how-to/Отладочный%20оверлей.md).
- **Releases are a button, not a merge.** `project.godot` holds the version being developed: it is bumped once at the start of a cycle with `bash .github/scripts/set_version.sh X.Y.Z` in a PR and never edited by hand; Actions → «Релиз» → Run workflow tags master and publishes it. Every other build — each PR, each push to master, `dev/release.ps1` → `dist/` — is numbered `X.Y.Z-dev.N+hash`, shown in the main-menu corner and the debug overlay, so a bug screenshot names its commit. Details: [Релизы и сборка](docs/astral-genesis/how-to/Релизы%20и%20сборка.md).
- **Cloud sessions** (Claude Code on the web) are set up by the SessionStart hook `.claude/hooks/session-start.sh` — git-lfs and LFS objects, the GECS submodule, tags, Godot, pwsh, a cold import; it does nothing on a local machine. Re-import after asset changes with `bash dev/import.sh`, never bare `godot --import`: multithreaded import hangs headless. [Релизы и сборка → Облачная сессия Claude](docs/astral-genesis/how-to/Релизы%20и%20сборка.md).
- **Godot MCP** — the **Beckett** editor plugin (`addons/beckett`, server `127.0.0.1:8770`, wired in `.mcp.json`) lets an agent see the live editor and running game: scene tree, node state, logs, screenshots. It exists only while the Godot editor is open; headless checks stay the source of truth. Its `BeckettRuntime` autoload is an inert stub outside the editor. [Редакторские инструменты → Beckett](docs/astral-genesis/how-to/Редакторские%20инструменты.md).
- **Assets** (`.glb`, `.svg`, images) are tracked via **Git LFS** — see [Конвенции проекта](docs/astral-genesis/Справка/Конвенции%20проекта.md).
- **The Blender side of the asset pipeline** is an add-on whose source lives in `tools/blender/` — it exports one `.glb` per collection and turns collision settings into real physics nodes on import. It is *linked* into Blender's extensions folder rather than copied (`tools/blender/install.ps1`), because two diverging copies is the failure mode that folder exists to prevent; tests: `tools/blender/tests/run_addon_tests.ps1`. `tools/` carries a `.gdignore` so Godot never scans it. [tools/blender/README.md](tools/blender/README.md).
...
**ECS** — the whole game is built on **GECS** (`addons/gecs/`), a git submodule pinned to `release-v9.2.0`; never edit files under it, they are upstream (`git submodule update --init` after cloning). Components (`C_*`) are pure data, systems (`S_*`) hold all behavior, entities (`E_*`) are containers, observers (`O_*`) react to component changes. The frame loop is driven manually in `src/world/main.gd` over named system groups. Three rules that cause silent breakage when violated: **order between systems is declared only through `deps()`** — GECS topologically sorts each group and moves a dependent system to the end, so scene node order guarantees nothing (`dev/system_order_check` guards the result); **raycasting systems must live in the `"physics"` group** (Jolt is threaded, space-state is only safe from `_physics_process`), and **structural changes — adding/removing components or entities — must go through the `cmd` buffer, never directly inside `process()`** (systems iterate archetypes zero-copy, so a direct change skips entities); writing component *fields* is unrestricted. Everything else — the full model, «Правило v9» in detail, ECS patterns and pitfalls: [GECS и правила движка](docs/astral-genesis/how-to/GECS%20и%20правила%20движка.md).

**World generation & run flow** — a run is a deterministic **graph** of rooms and **corridor branches** generated from a seed, streamed a whole layer at a time by the `RunManager` autoload; the save holds the seed, a snapshot of the generation knobs (`data/world_gen_config.tres`) and progress, never the rooms, which are regenerated. **A room's scene is chosen before its edges** — it gets exactly as many edges as its scene has doors — and hangs on one corridor branch assembled at spawn from a tile kit; the current node changes by the player's presence, not by pressing a door. **Layout never sees the world**: the planner works in cells and sides of the grid in `src/grid/` (topology, embedding, router — no game dependencies), and the cell size lives only in the layer plan's embedding — never write metres or a cell-size constant into layout code. Physical doors are bound to graph edges at spawn time rather than authored. Which room scene lands on a node is decided by **two independent axes that must never be merged**: structural `tags` («what the room can do» — a hard subset filter) and `room_type` («what kind of place it is» — a soft preference from `data/room_type_catalog.tres`, with per-depth ranges). Putting a type into `tags` turns it into a hard requirement — that is what once stopped `lab_room` from ever appearing. What each tag *means* is written down in a **descriptive-only** dictionary (`data/room_tag_catalog.tres`): the selector never reads it, so a tag missing from it still works — making it authoritative would turn «add a tag» in the editor tools into a silent break. [Цикл забега](docs/astral-genesis/how-to/Цикл%20забега.md).
