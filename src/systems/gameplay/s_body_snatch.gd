...
func _embody(soul: Entity, body: Entity, form: C_BodyForm) -> void:
	...
	#
	# Тем же вызовом ставим контрольную точку, и это не «на всякий случай»:
	# mark_body_consumed выше уже записал тело съеденным НА ДИСК, а воплощение
	# игрока до ближайшей смены комнаты нигде не сохранялось. Вылет в этом окне
	# давал сейв, где тело поглощено, а игрок призрак, — тело исчезало из мира
	# насовсем, не достав никому. Точка обязана сниматься ПОСЛЕ применения
	# буфера: до него _checkpoint прочитал бы душу ещё развоплощённой и записал
	# ровно то расхождение, от которого мы уходим.
	cmd.add_custom(
		func():
