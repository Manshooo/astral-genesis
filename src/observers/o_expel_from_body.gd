...
static func expel(soul: Entity, voluntary: bool) -> void:
	...
	# Контрольная точка на выходе из тела — по той же причине, что и на входе
	# (см. S_BodySnatch._embody): развоплощение переписывает и запас распада, и
	# воплощение, а до ближайшей смены комнаты в сейве этого нет. Здесь мы уже
	# вне буфера — expel зовётся отложенно, компоненты сняты по-настоящему.
