...
##
## Пока читается только коридорным путём генератора (флаг corridors): старый
## путь живёт на своих константах и обязан давать ровно тот же граф, что и до
## появления этого ресурса, — на нём держатся сейвы и все проверки.
@tool  # читается генератором и в редакторской вкладке «Генератор мира»
class_name RS_WorldGenConfig
extends Resource

## Коридорный путь генерации (карточка «Процедурные коридоры между
## комнатами»). Выключен, пока тайлы коридоров не собираются в игре: узел-
## коридор без геометрии заспавнить нечем.
@export var corridors: bool = false
