...
## Тип помещения (ключ из RS_RoomTypeCatalog): что это за комната — арсенал,
## лаборатория, склад. Пустой ключ = безликое помещение.
##
## ОТДЕЛЬНОЕ поле, а не ещё один тег в `tags`, и это принципиально. `tags` —
## жёсткий фильтр вместимости: узел получит только пресеты, чьи теги покрывают
## его собственные, а среди прошедших выигрывает тот, у кого лишних тегов
## меньше. Тип, попав туда же, начал бы работать требованием («узел-арсенал
## получит ТОЛЬКО арсеналы»), и заодно сломал бы специфичность: пресет с типом
## всегда «богаче» на один тег и проигрывал бы безликому на узлах без типа.
## Ровно на этом однажды перестали выпадать lab_room и exit_room.
##
## Правило подбора у типа своё и мягкое — см. RS_RoomPresetLibrary._prefer_room_type.
