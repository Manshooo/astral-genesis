...
## Итоги последнего ЗАВЕРШЁННОГО забега — их показывает экран итогов. Лежат в
## прохождении, а не в группе «Прогресс забега» ниже, и `clear_run()` их не
## трогает: забег кончился, но его сводка остаётся тем, что игрок унёс.
@export var last_run: RS_RunStats
...
## Статистика идущего забега (RunStats.current — ТОТ ЖЕ объект). Лежит в сейве,
## а не только в автолоаде, ровно по той же причине, что и остальной прогресс:
## «выход в меню» забег не заканчивает, и без этого поля возврат в комплекс
## показывал бы в итогах пустую сводку вместо получаса игры.
@export var run_stats: RS_RunStats
## Ручки, по которым сгенерирован ЭТОТ забег (снимок RS_WorldGenConfig на его
## старте). Сид задаёт комплекс только вместе с ручками: без снимка улучшение
## Архитектора, купленное посреди забега, на загрузке перестроило бы комплекс
## под игроком. null = забег начат до появления снимков — берётся текущий конфиг.
@export var gen_config: RS_WorldGenConfig
...
## Узлы, чью разовую награду забега игрок уже забрал (встреча с Архитектором).
## По той же причине, что съеденные тела: комната восстанавливается из сида, и
## без пометки выход в меню и загрузка давали бы награду заново.
@export var rewarded_node_ids: Array[StringName] = []
...
func clear_run() -> void:
	...
	# Именно ОТВЯЗЫВАЕМ, а не чистим на месте: снятый снимок (RunStats.last)
	# держит этот же объект, и очистка полей стёрла бы сводку забега ровно в тот
	# момент, когда её собрались показать (record_death зовётся сразу за finish).
	run_stats = null
	# Следующий забег генерируется по тем ручкам, что будут на его старте.
	gen_config = null
