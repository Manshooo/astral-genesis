## res://src/resources/rs_game_config.gd
## Конфиг для разработчика: значения баланса/физики, которые НЕ являются
## пользовательскими настройками и не должны попадать в user:// или в
## draft/apply/reset флоу SettingsManager. Редактируется напрямую в
## инспекторе на data/game_config.tres.
class_name RS_GameConfig
extends Resource

@export_group("FPS")
@export var pitch_limit: float = 1.4           ## ~80° в радианах

@export_group("Physics")
@export var gravity: float = 9.8

## Высота, ниже которой сущность считается провалившейся из мира насовсем, м.
## Ниже пола любой комнаты и любого слоя: провалиться сюда можно только сквозь
## дырку в геометрии, и подниматься обратно неоткуда. См. S_VoidFall.
@export var void_fall_depth: float = -100.0

# Числа полёта БФЖ жили здесь, пока полёт был веткой внутри системы движения.
# Теперь полёт — возможность (C_Flight на сцене e_player.tscn), и числа лежат
# вместе с ней: «сделать полёт быстрее» правится в одном месте, а не в двух.

@export_group("Распад")
## Во сколько раз быстрее утекает запас СВЕРХ собственного максимума души —
## тот излишек, что принесён из тела при добровольном выходе. Смысл в том, что
## накопить огромный буфер и ходить с ним нельзя: излишек надо тратить.
@export var lifespan_overflow_leak: float = 3.0
## Какая доля собственного максимума остаётся душе, если тело ПОГИБЛО (HP в ноль
## или его время вышло). Сгорело — значит сгорело: остаток тела не переносится,
## и запас души не «сохраняется», а падает до этой доли.
@export_range(0.0, 1.0) var lifespan_death_fraction: float = 0.1

## Сколько секунд гаснет экран между распадом БФЖ и итогами забега. Не «красиво
## подольше»: всё это время мир ещё живой (см. RunManager.die), и каждая лишняя
## доля секунды — это время, за которое по уже мёртвому игроку успевают ударить.
@export_range(0.0, 3.0) var death_fade_duration: float = 0.8

@export_group("Сохранение")
## Через сколько секунд после последней контрольной точки ставится следующая
## (RunManager._process). Верхняя граница того, сколько игрок теряет при вылете:
## внутри комнаты распад и урон иначе не попадают в сейв вовсе. Не меньше — запись
## идёт на диск целым ресурсом, и делать это каждые пару секунд незачем.
@export_range(5.0, 120.0) var autosave_interval: float = 20.0

@export_group("Архитектор")
## Эссенции за встречу с Архитектором — один раз за комнату за забег. Единственный
## пока источник валюты его улучшений; детали в комнатах станут вторым, той же
## валютой.
@export_range(0, 10) var architect_essence_reward: int = 1

@export_group("World generation")
## Библиотека пресетов комнат для процедурной генерации.
## Пусто = все узлы получают placeholder-комнату (поведение до появления
## библиотеки). Назначается в инспекторе на data/game_config.tres.
@export var room_preset_library: RS_RoomPresetLibrary
## Ручки генерации (data/world_gen_config.tres). Базовые: забег берёт СНИМОК
## (см. RunManager._run_gen_config), и правка ресурса идёт в силу со следующего
## забега, а не посреди начатого. Пусто = ручки по умолчанию генератора.
@export var world_gen: RS_WorldGenConfig
## Куски коридора (data/corridor_kit.tres): чем RunManager застраивает тайлы
## коридорной раскладки.
@export var corridor_kit: RS_CorridorKit
