...
##
## СОГЛАШЕНИЕ О ВЗГЛЯДЕ: тело смотрит в −Z, как всё в Godot (камера, Vector3.FORWARD,
## луч взаимодействия). Экспорт из Blender разворачивает модель лицом в +Z, поэтому
## внутри сцены она довёрнута на 180° — доворот обязан лежать ПОД корнем (на
## арматуре), иначе visual_of его не увидит: он читает трансформ меша относительно
## корня. Тело, авторенное лицом назад, при вселении смотрело бы игроку в спину.
@tool
class_name E_Body
extends Entity
...
## направление ригу задаёт мышь игрока, а не поза трупа. А вот поворот МОДЕЛИ
## внутри сцены попадает — тем и задаётся, куда тело смотрит лицом (см. ниже про
## соглашение о взгляде).
##
## Подошву облика считаем по ФОРМЕ того же тела: меш и габарит обязаны сесть на
## один пол, а спросить об этом рига в момент надевания нельзя (см.
## C_BodyVisual.foot_offset).
##
...
static func visual_of(body: Node) -> C_BodyVisual:
	...
	visual.foot_offset = form.foot_offset() if form else Vector3.ZERO
	return visual
