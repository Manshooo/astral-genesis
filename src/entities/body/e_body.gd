...
##
## Трансформ берём относительно КОРНЯ тела, и это же и есть «относительно
## подошвы»: у всех сцен тел origin стоит в ступнях (коллизионная капсула поднята
## на половину своей высоты, дном на y = 0). Собственный поворот тела в комнате
## сюда не попадает — направление ригу задаёт мышь игрока, а не поза трупа.
##
