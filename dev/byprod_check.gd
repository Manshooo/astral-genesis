...
##
## САМ БИНДИНГ при этом всё ещё собран под 0.5.2 и честно предупреждает об этом в
## лог при старте. На 0.5.3 проверено прогоном: все остальные ассерты проходят —
## события находятся, инстанс играет, пауза работает, — то есть пара совместима.
## Пересобрать биндинг под 0.5.3 нужно в godot-byprod, отсюда это не делается.
