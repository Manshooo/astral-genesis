...
## работать как тег — жёстким фильтром, — и комнаты с типом либо перестанут
## выпадать вовсе, либо начнут требоваться жёстко. Ни то, ни другое не бросает
## ошибку: генератор просто выдаёт другой комплекс.
##
...
func _check_selection() -> void:
	...
	# Тип не должен подменять собой жёсткий фильтр: пресет своего типа, но без
	# портала, обязан отсеяться у узла, которому портал нужен, как и любой другой.
	var arsenal_no_portal := _make_preset("Арсенал без портала", &"arsenal", [])
	var plain_portal := _make_preset("Безликая с порталом", &"", [&"vertical_hub"])
	var library2 := _make_library([arsenal_no_portal, plain_portal])
	var portal_node := _make_node(&"arsenal", [&"vertical_hub"])
	_check(
		"тип не пробивает фильтр портала",
		_always_picks(library2, portal_node, rng, plain_portal),
		"узлу с вертикальным переходом досталась комната без портала — тип обошёл жёсткий фильтр",
	)

	# Лишние теги пресета узлу не мешают: специфичности больше нет, и пресет
	# своего типа с тегом, которого узел не просил, не проигрывает безликому.
	var typed_extra := _make_preset("Арсенал с меткой", &"arsenal", [&"hazard"])
	var plain_bare := _make_preset("Безликая", &"", [])
	var library3 := _make_library([typed_extra, plain_bare])
	_check(
		"лишний тег пресета не отнимает у него узел своего типа",
		_always_picks(library3, _make_node(&"arsenal", []), rng, typed_extra),
