...
|---|---|---|
| **Воплощён** | `C_Embodied` + `C_Health` | здоровье тела; распад **замедлен** (`embodied_rate`, по умолчанию ×0.25); берёт урон |
| **Развоплощён** (призрак) | нет `C_Embodied`/`C_Health` | тикает `C_Lifespan` на полной скорости; урон не берёт; истечение = смерть |
...
| `C_Lifespan` | `E_Player` (идентичность) | `max_duration` (60), `current`, `embodied_rate` (0.25) |
